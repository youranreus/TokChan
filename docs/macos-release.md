...
每项用户能感知的新增、优化或修复都要随代码提交一个永久片段，路径格式为 `release-notes/fragments/YYYYMMDD-lowercase-slug.json`：
...
分类只能是“新增”“优化”“修复”。`summary` 应用一句简体中文描述用户得到的结果，不写提交标题、实现细节或营销文案；必须为去除首尾空白的单行 NFC 文本，包含中文，长度为 8–80 个字符，且不能包含 HTML 标签。一个可独立说明的用户结果对应一个片段。测试、文档、重构、发布基础设施、版本提交和 Trellis 归档不创建片段。

片段一经出现在稳定 Tag 中就不得修改、改名或删除。发布生成器只选择上一稳定祖先 Tag 到当前 Tag 间新增的片段，按“新增 → 优化 → 修复”和文件名排序，同时生成 GitHub Markdown、Sparkle HTML 与私有校验清单。没有新片段、格式非法或历史片段被更改时，Tag 工作流会在凭据和 Release 操作前失败，不会生成占位说明。

Tag 创建前应在代码提交中修正尚未发布的片段，再重新准备版本。Tag 已发布后不得移动 Tag 或改写 Release；文案问题通过新的 patch 版本和新片段说明。草稿正文会按 Tag 源码自动校正，已发布正文只读校验。历史 Release 保持不变，直接绕过 Trellis 的人工提交不会由常规 PR/push CI 机械拦截。
