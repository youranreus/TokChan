...
- Before packaging, strict `codesign --verify --deep --strict --verbose=2` verification and `codesign -dv --verbose=4` inspection must prove the expected designated identifier, the selected signature mode, Info.plist coverage, and sealed resources. For each nested Sparkle component, parse exactly one native `codesign -dr -` requirement record: Developer ID signatures emit `designated => ...`, while ad-hoc signatures may emit the implicit `# designated => ...` form. Official mode additionally requires the expected Developer ID authority, Team ID, secure timestamp, and hardened runtime.
