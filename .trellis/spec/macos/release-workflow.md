...
- The Actions workflow uses the platform token through `GH_TOKEN: ${{ github.token }}` and declares only `contents: write`, `pages: write`, and `id-token: write`. Pin checkout and Pages actions by full commit SHA, keep same-Tag concurrency with cancellation disabled, and deploy the production feed through the `github-pages` environment. The deployment step exposes its `page_url`; do not add pull-request or untrusted-fork release triggers.
